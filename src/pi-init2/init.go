...
func copyAppliance(path string, info os.FileInfo, err error) error {
  ...
  if (info.IsDir()) {
    ...
    // Store permissions on copy
    err = os.Chmod(dst, si.Mode())
    check(err)

    fmt.Println("Coppied "+path+" from /boot/appliance")
  }
...
func replaceCmdline() {
  /* Rename cmdline.original.txt to cmdline.txt if exists */
  if _, err := os.Stat("/boot/cmdline.original.txt"); !os.IsNotExist(err) {
    checkFatal(
      "replacing cmdline",
      os.Rename("/boot/cmdline.original.txt", "/boot/cmdline.txt"))
  }
}
